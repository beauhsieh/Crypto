...
public class CorsInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if(null != request.getHeader("Content-Type")){
			response.setHeader("Accept-Patch", request.getHeader("Content-Type"));
		}
		response.setHeader(AC_ALLOW_ORIGIN, origin);
		return true;
	}
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
		if(this.corsData.isSimple()) {
			//Add Access-Control-Allow-Origin is not work, even remove if condition. so add the follow line to preHandle.
			response.setHeader(AC_ALLOW_ORIGIN, origin);
		}
	}
